...
4. **M3 — Scene decomposition + object marks.** *(Redefined 2026-07 — see docs/prior-art.md.)* The missing layer between pixels and marks is a **scene plan**: true objects with boundaries, depth order, and committed per-surface tone. (a) `decompose/` offline stage: SAM + DepthAnything freeze a plan (`<photo>.scene.{npz,json,png}`) next to each source ✅; (b) engine consumes it — `scene` zone selector, zone-level `base` pass (one committed directional treatment per surface), `keyline_mm` white seams ✅; (c) object-glyph modules (tree stamps, cloud lobes, rock facets placed in depth order, occluding) — pending; (d) second plan front-end: AI-generated engraving raster → extract tone/orientation/regions from the *stylized* image (the model does the artistic simplification), engine re-inks with clean strokes — pending. Procedural generators and source genes fold into (c)/(d).
